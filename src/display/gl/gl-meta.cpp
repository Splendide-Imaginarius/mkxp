...
static void _blitBegin(FBO::ID fbo, const Vec2i &size)
{
	...
	else
	{
		...
		// High-res: if the target framebuffer isn't the screen, make
		// sure that we don't misdetect the destination texture as the screen.
		if (fbo != FBO::ID(0)) {
			glState.allowFramebufferScaling.pushSet(false);
		}
		else {
			glState.allowFramebufferScaling.pushSet(glState.allowFramebufferScaling.get());
		}
...
void blitEnd()
{
	...
	if (!HAVE_NATIVE_BLIT) {
		...
		// Resume the usual high-res framebuffer scaling.
		glState.allowFramebufferScaling.pop();
	}
}
